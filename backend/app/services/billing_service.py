...
def calculate_tenant_monthly_cost(db: Session, tenant_id: UUID) -> float:
    ...
    total_cost = 0.0
    for input_tokens, output_tokens, model_string, provider, input_price, output_price in rows:
        # If input price is configured, calculate input cost, otherwise 0
        input_cost = (input_tokens / 1_000_000.0 * float(input_price)) if input_price is not None else 0.0
        # If output price is configured, calculate output cost, otherwise 0
        output_cost = (output_tokens / 1_000_000.0 * float(output_price)) if output_price is not None else 0.0
