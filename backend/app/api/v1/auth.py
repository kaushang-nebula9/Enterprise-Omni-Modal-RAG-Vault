...
def register_signup(
    request: RegistrationInitRequest, response: Response, db: Session = Depends(get_db)
):
...
def forgot_password_route(
    ...
    try:
        ...
        send_forgot_password_otp_email(
            to_email=request.email, full_name=user.full_name, otp=raw_otp
        )
...
def invite_member(
    ...
    role = (
        db.query(Role)
        .filter(Role.id == request.role_id, Role.tenant_id == current_admin.tenant_id)
        .first()
    )
    if not role:
        raise HTTPException(
...
def accept_invite(request: AcceptInviteRequest, db: Session = Depends(get_db)):
    ...
    invite_token = (
        db.query(InviteToken).filter(InviteToken.token_hash == hashed_token).first()
    )
    ...
    if expires_at < current_time:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Invite token has expired"
        )
...
async def google_auth_callback(
    ...
    if user:
        ...
        return RedirectResponse(
            url=f"{settings.FRONTEND_URL}/register/google-org?setup_token={token}"
        )
...
def google_complete_setup(
    request: GoogleOrgSetupRequest, response: Response, db: Session = Depends(get_db)
):
