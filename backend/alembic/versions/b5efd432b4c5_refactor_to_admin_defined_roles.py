...
def upgrade() -> None:
    ...
    op.add_column(
        "document_access_policies", sa.Column("role_id", sa.Uuid(), nullable=True)
    )
    ...
    for tenant in tenants:
        ...
        bind.execute(
            sa.text(
                "UPDATE users SET role_id = :role_id WHERE tenant_id = :tenant_id AND role = 'admin'"
            ),
        ...
        bind.execute(
            sa.text(
                "UPDATE users SET role_id = :role_id WHERE tenant_id = :tenant_id AND role = 'member'"
            ),
    ...
    op.drop_constraint(
        "uq_document_id_role", "document_access_policies", type_="unique"
    )
    op.create_unique_constraint(
        "uq_document_id_role_id", "document_access_policies", ["document_id", "role_id"]
    )
    op.create_foreign_key(
        None,
...
def downgrade() -> None:
    """Downgrade schema."""
    op.add_column(
        "document_access_policies", sa.Column("role", sa.String(), nullable=True)
    )
    ...
    op.drop_constraint(
        "uq_document_id_role_id", "document_access_policies", type_="unique"
    )
    op.create_unique_constraint(
        "uq_document_id_role", "document_access_policies", ["document_id", "role"]
    )
