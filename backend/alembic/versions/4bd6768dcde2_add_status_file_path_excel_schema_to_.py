...
document_status_enum = sa.Enum('pending', 'processing', 'ready', 'failed', name='documentstatus')
...
def upgrade() -> None:
    ...
    op.add_column('documents', sa.Column('status', sa.Enum('pending', 'processing', 'ready', 'failed', name='documentstatus'), server_default='pending', nullable=False))
    op.add_column('documents', sa.Column('file_path', sa.String(), nullable=True))
    op.add_column('documents', sa.Column('excel_schema', postgresql.JSONB(astext_type=sa.Text()), nullable=True))
