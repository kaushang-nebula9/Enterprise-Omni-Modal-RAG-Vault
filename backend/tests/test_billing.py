...
def test_check_tenant_budgets_task_warning(mock_session_local):
    ...
    with patch("app.tasks.billing_tasks.calculate_tenant_monthly_cost", return_value=120.00) as mock_cost_calc:
        with patch("app.tasks.billing_tasks.logger") as mock_logger:
            ...
            assert "exceeded monthly budget limit" in mock_logger.warning.call_args[0][0]
...
def test_check_tenant_budgets_task_notification(mock_session_local, db):
    ...
    with patch.object(db, "close", return_value=None):
        with patch("app.tasks.billing_tasks.calculate_tenant_monthly_cost", return_value=15.00):
            check_tenant_budgets_task()
