...
def test_query_message_model_relationship(db):
    ...
    db_message = db.query(QueryMessage).filter(QueryMessage.session_id == session_id).first()
...
def test_model_update_and_deactivate(db):
    ...
    # Verify updates
    updated_model = db.query(AvailableModel).filter(AvailableModel.id == model_id).first()
