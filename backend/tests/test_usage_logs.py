...
async def test_run_rag_pipeline_openrouter_success(db):
    ...
    with patch("app.services.embedding_service.embed_text", side_effect=mock_embed_text), \
         patch("app.services.rag_service.search_vectors", side_effect=mock_search_vectors), \
         patch("app.services.rag_service._get_cross_encoder", return_value=MockCrossEncoder()), \
         patch("app.services.openrouter_service.stream_openrouter_completion", side_effect=mock_stream_openrouter_completion):
...
async def test_run_rag_pipeline_failed_no_log(db):
    ...
    mock_client = MockAnthropicClient()
    mock_client.messages.stream = MagicMock(side_effect=Exception("Anthropic API Error"))
