...
def test_parse_summarize_no_question(db):
    ...
    # Without attachment
    disp, ret, inst, cmp_ids, is_cmp, is_sum = parse_chat_command("/summarize", db, user, None)
    ...
    # With attachment
    doc_id = uuid.uuid4()
    disp, ret, inst, cmp_ids, is_cmp, is_sum = parse_chat_command("/summarize", db, user, doc_id)
    assert disp == "/summarize"
...
def test_parse_other_prefix_commands(db):
    ...
    # Detailed
    disp, ret, inst, cmp_ids, is_cmp, is_sum = parse_chat_command("/detailed How does it work?", db, user, None)
    ...
    # Table
    disp, ret, inst, cmp_ids, is_cmp, is_sum = parse_chat_command("/table list of users", db, user, None)
    assert disp == "/table list of users"
    assert "table" in inst

    # Bullets
    disp, ret, inst, cmp_ids, is_cmp, is_sum = parse_chat_command("/bullets items to buy", db, user, None)
    assert disp == "/bullets items to buy"
    assert "bullet points" in inst

    # ELI5
    disp, ret, inst, cmp_ids, is_cmp, is_sum = parse_chat_command("/eli5 quantum computing", db, user, None)
...
def test_parse_compare_documents(db):
    ...
    disp, ret, inst, cmp_ids, is_cmp, is_sum = parse_chat_command(
        "/compare [ReportA.pdf] [ReportB.pdf] What are the differences in revenue?", db, user, None
    )
    assert disp == "/compare [ReportA.pdf] [ReportB.pdf] What are the differences in revenue?"
...
def test_parse_multiple_commands(db):
    ...
    disp, ret, inst, cmp_ids, is_cmp, is_sum = parse_chat_command("/detailed /table /bullets What are the requirements?", db, user, None)
    ...
    disp, ret, inst, cmp_ids, is_cmp, is_sum = parse_chat_command("/detailed /bullets", db, user, None)
...
def test_parse_compare_with_other_command(db):
    ...
    disp, ret, inst, cmp_ids, is_cmp, is_sum = parse_chat_command(
        "/compare [ReportA.pdf] [ReportB.pdf] /bullets /table Compare revenues", db, user, None
    )
    assert disp == "/compare [ReportA.pdf] [ReportB.pdf] /bullets /table Compare revenues"
