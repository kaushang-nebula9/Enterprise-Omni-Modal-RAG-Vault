...
export const documentService = {
  ...
  uploadDocument: async (
    ...
    uncheckedAncestorIds.forEach((id) =>
      formData.append("unchecked_ancestor_ids", id),
    );
  ...
  previewAssignment: async (
    ...
    const response = await api.get("/api/v1/documents/preview-assignment", {
      params,
    });
    return response.data;
  },
  ...
  assignDepartment: async (
    ...
    const response = await api.post(
      `/api/v1/documents/${documentId}/assign-department`,
      {
  ...
  uploadPersonalDocument: async (file: File): Promise<DocumentResponse> => {
    ...
    formData.append("file", file);
    const response = await api.post(
      "/api/v1/personal-documents/upload",
      formData,
      {
        headers: { "Content-Type": "multipart/form-data" },
      },
  ...
  downloadPersonalDocument: async (
    documentId: string,
    filename: string,
  ): Promise<void> => {
    const response = await api.get(
      `/api/v1/personal-documents/${documentId}/download`,
      {
        responseType: "blob",
      },
    );
  ...
  deletePersonalDocument: async (
    documentId: string,
  ): Promise<MessageResponse> => {
    const response = await api.delete(
      `/api/v1/personal-documents/${documentId}`,
    );
    return response.data;
  },
};
