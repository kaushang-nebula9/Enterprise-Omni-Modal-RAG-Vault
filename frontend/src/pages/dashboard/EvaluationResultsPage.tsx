...
const QueryResultCard: React.FC<{
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all flex flex-col gap-4 relative overflow-hidden">
      {/* Header & Toggle area */}
      <div
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span
            className={`text-[10px] font-semibold px-2.5 py-0.5 rounded-md font-sans border ${
        ...
        <div className="flex items-center gap-3">
          <div
            className={`flex items-center gap-1.5 border px-2.5 py-1 rounded-xl text-xs font-bold font-sans ${getScoreColor(result.faithfulness_score)}`}
          >
          ...
          <div
            className={`flex items-center gap-1.5 border px-2.5 py-1 rounded-xl text-xs font-bold font-sans ${getScoreColor(result.relevance_score)}`}
          >
            <span>Relevance:</span>
      ...
      {isOpen && (
        <div className="flex flex-col gap-4">
              ...
              {result.answer ? (
                <ReactMarkdown
                  ...
                  components={{
                    ...
                    tbody: ({ ...props }) => (
                      <tbody
                        className="divide-y divide-slate-100 dark:divide-slate-850"
                        {...props}
                      />
                    ),
                    tr: ({ ...props }) => (
                    ...
                    th: ({ ...props }) => (
                      <th
                        className="px-3 py-2 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider"
                        {...props}
                      />
                    ),
                    td: ({ ...props }) => (
                      <td
                        className="px-3 py-2 text-sm text-slate-700 dark:text-slate-205"
                        {...props}
                      />
                    ),
                    p: ({ ...props }) => (
              ...
              <div className="bg-red-50/20 dark:bg-red-950/5 border border-red-100 dark:border-red-900/30 rounded-xl p-4 flex flex-col gap-2">
                <div className="flex items-center gap-2 text-red-650 dark:text-red-400">
                  ...
                  <span className="text-xs font-bold uppercase tracking-wider font-sans">
                    Claims Unsupported by Source Context (
                ...
                <ul className="list-disc pl-5 flex flex-col gap-1">
                  {result.unsupported_claims.map((claim, idx) => (
                    <li
                      key={idx}
                      className="text-xs text-red-650 dark:text-slate-400 leading-relaxed font-sans font-medium"
                    >
                      {claim}
                    </li>
                  ))}
          ...
          <div className="bg-slate-50 dark:bg-slate-950 border border-slate-150 dark:border-slate-900 rounded-xl p-4 flex flex-col gap-1.5">
            <span className="text-xs font-bold text-indigo-500 uppercase tracking-wider font-sans">
              Judge Reasoning
            </span>
...
const EvaluationResultsPage: React.FC = () => {
  ...
  const [viewMode, setViewMode] = React.useState<"run" | "all">(
    defaultViewMode,
  );
  ...
  const { data: allResultsData, isLoading: isAllResultsLoading } = useQuery({
    ...
    queryFn: () =>
      evaluationService.getAllEvaluationResults(limit, (page - 1) * limit),
  ...
  const sortedModelPerformance = React.useMemo(() => {
    ...
    return [...modelPerformance].sort(
      (a, b) => b.avg_faithfulness_score - a.avg_faithfulness_score,
    );
  }, [modelPerformance]);
  ...
  if (error || !detail) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-4 text-center max-w-md mx-auto">
        ...
        <h3 className="font-sora text-lg font-bold text-slate-800 dark:text-slate-100">
          Error Loading Details
        </h3>
        <p className="text-sm text-slate-500 dark:text-slate-400 font-sans">
          Could not retrieve results for this evaluation run. It might not exist
  ...
  const getScoreColor = (score: number) => {
    if (score >= 80)
      return "text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/40 border-emerald-100 dark:border-emerald-900/40";
    if (score >= 50)
      return "text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/40 border-amber-100 dark:border-amber-900/40";
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col gap-0.5">
              ...
              <span className="text-xs text-slate-400 font-sans">
                {viewMode === "run"
                  ? `Run ID: ${run.id}`
                  : "All evaluation runs across your vault"}
              </span>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1">
            <span className="text-4xl font-bold font-sora text-slate-800 dark:text-slate-100">
              {viewMode === "run"
                ? run.avg_faithfulness_score !== null &&
                  run.avg_faithfulness_score !== undefined
          ...
          <p className="text-slate-500 dark:text-slate-400 text-xs leading-relaxed font-sans mt-1">
            Measures if the generated answers make any claims that are not fully
            supported by the retrieved contexts. Higher means more factual.
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1">
            <span className="text-4xl font-bold font-sora text-slate-800 dark:text-slate-100">
              {viewMode === "run"
                ? run.avg_relevance_score !== null &&
                  run.avg_relevance_score !== undefined
          ...
          <p className="text-slate-500 dark:text-slate-400 text-xs leading-relaxed font-sans mt-1">
            Measures if the retrieved chunks are actually relevant and useful
            for answering the question. Higher means less noise.
          </p>
        </div>
                ...
                <span className="font-semibold text-slate-850 dark:text-slate-100 font-sans text-xs">
                  {viewMode === "run"
                    ? run.date_range_start
                      ? `${new Date(run.date_range_start).toLocaleDateString()} to ${new Date(run.date_range_end || "").toLocaleDateString()}`
                      : "None (Latest count)"
                    : "All Time"}
                </span>
              </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-150 dark:border-slate-800 text-xs font-bold uppercase tracking-wider text-slate-400">
                  ...
                  <th className="py-3 px-4 text-center font-sans">
                    Avg Faithfulness
                  </th>
                  <th className="py-3 px-4 text-center font-sans">
                    Avg Relevance
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50">
                {sortedModelPerformance.map((row) => {
                  ...
                  return (
                    <tr
                      key={row.model_string}
                      className="transition-colors text-sm text-slate-700 dark:text-slate-300"
                    >
                      ...
                      <td className="py-3.5 px-4 text-center font-semibold">
                        <span
                          className={`px-2.5 py-0.5 rounded-full text-xs font-bold border ${getScoreColor(row.avg_faithfulness_score)}`}
                        >
                      ...
                      <td className="py-3.5 px-4 text-center font-semibold">
                        <span
                          className={`px-2.5 py-0.5 rounded-full text-xs font-bold border ${getScoreColor(row.avg_relevance_score)}`}
                        >
        ...
        <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-2">
          <h3 className="font-sora text-lg font-semibold text-slate-800 dark:text-slate-100 flex items-center gap-2">
            ...
            {viewMode === "run"
              ...
              : `All Evaluated Queries (${allResultsData?.total_count || 0})`}
          </h3>
        ...
        {viewMode === "run" ? (
          results.length === 0 ? (
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-8 text-center flex flex-col items-center justify-center gap-2">
              ...
              <span className="font-semibold text-slate-800 dark:text-slate-205 font-sans">
                No evaluated queries found
              </span>
              <span className="text-xs text-slate-500 dark:text-slate-450 font-sans">
                Make sure there are queries logged in your vault before running
                evaluations.
              </span>
            </div>
          ) : (
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-8 text-center flex flex-col items-center justify-center gap-2">
            ...
            <span className="text-xs text-slate-500 dark:text-slate-450 font-sans">
              No evaluation results have been logged across any runs yet.
            </span>
          </div>
        ) : (
          <div className="flex flex-col gap-6">
                ...
                <button
                  onClick={() =>
                    setPage((p) =>
                      Math.min(
                        p + 1,
                        Math.ceil(allResultsData.total_count / limit),
                      ),
                    )
                  }
                  disabled={
                    page >= Math.ceil(allResultsData.total_count / limit)
                  }
                  className="px-3 py-1.5 text-xs font-semibold rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-705 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 disabled:opacity-50 transition-colors cursor-pointer"
                >
                  Next
