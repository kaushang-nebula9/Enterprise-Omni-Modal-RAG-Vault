...
export const RolesPermissionsPage: React.FC = () => {
  ...
  const filteredRoles = useMemo(() => {
    return (
      roles?.filter((role: RoleResponse) => {
        if (
      ...
      <div className="flex justify-between items-center shrink-0">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
            Define roles for your organisation. Document access is configured
            per document during upload.
          </p>
        </div>
        <button
            ...
            <div className="flex flex-wrap items-center justify-end gap-3 w-full lg:w-auto shrink-0">
              ...
              >
                ...
                >
                  ...
                  <ChevronDown
                    className={`w-4 h-4 text-slate-400 transition-transform duration-200 ${isDeptDropdownOpen ? "rotate-180" : ""}`}
                  />
                </button>
              ...
              >
                ...
                {isTypeDropdownOpen && (
                  <div className="absolute right-0 mt-1.5 w-40 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl z-30 overflow-hidden py-1">
                    {(["all", "admin", "default", "custom"] as const).map(
                      (typeVal) => (
                        <button
                          key={typeVal}
                          onClick={() => {
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                    {filteredRoles.length === 0 ? (
                      <tr>
                        <td
                          colSpan={5}
                          className="text-center text-slate-400 dark:text-slate-500 py-16"
                        >
                      ...
                      filteredRoles.map((role: RoleResponse) => {
                        const parentRole = role.parent_role_id
                          ? roles?.find(
                              (r: RoleResponse) => r.id === role.parent_role_id,
                            )
                          : null;
                        return (
                          <tr
                          ...
                          >
                            ...
                            <td className="px-4 py-3.5">
                              <div className="flex items-center gap-2">
                                ...
                                {role.is_admin && (
                                  <span className="inline-block bg-indigo-100 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-400 rounded-full px-2 py-0.5 text-[10px] font-medium">
                                    Admin
                                  </span>
                                )}
                                {role.is_default && !role.is_admin && (
                                  <span className="inline-block bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-full px-2 py-0.5 text-[10px] font-medium">
                                    Default
                                  </span>
                                )}
                              </div>
                            ...
                            <td className="px-4 py-3.5 text-slate-500 dark:text-slate-400">
                              {parentRole ? (
                                ...
                                <span className="text-xs italic text-slate-400 dark:text-slate-550">
                                  Top-level role
                                </span>
                              )}
                            </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in-95 duration-200 text-slate-800 dark:text-slate-100">
            <div className="flex justify-between items-center p-6 border-b border-slate-100 dark:border-slate-800">
              <h2 className="text-lg font-semibold font-sora text-slate-800 dark:text-slate-100">
                Create Role
              </h2>
              <button
            ...
            <form
              onSubmit={(e) => {
                ...
                createMutation.mutate({
                  name: newRoleName,
                  parent_role_id: newParentRoleId || null,
            ...
            >
              ...
              <div className="space-y-1">
                ...
                <select
                  ...
                  onChange={(e) => setNewParentRoleId(e.target.value || null)}
                  className="w-full px-4 py-2 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-indigo-100 dark:focus:ring-indigo-950/50 focus:border-indigo-500 dark:focus:border-indigo-400 text-slate-800 dark:text-slate-100 outline-none transition-all"
                >
              ...
              <div className="space-y-1">
                ...
                <select
                  ...
                  onChange={(e) => setNewDepartmentId(e.target.value || null)}
                  className="w-full px-4 py-2 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-indigo-100 dark:focus:ring-indigo-950/50 focus:border-indigo-500 dark:focus:border-indigo-400 text-slate-800 dark:text-slate-100 outline-none transition-all"
                >
              ...
              onSubmit={(e) => {
                ...
                updateMutation.mutate({
                  ...
                  data: {
                    ...
                    parent_role_id: editParentRoleId || null,
            ...
            >
              ...
              <div className="space-y-1">
                ...
                <select
                  ...
                  onChange={(e) => setEditParentRoleId(e.target.value || null)}
                  className="w-full px-4 py-2 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-indigo-100 dark:focus:ring-indigo-950/50 focus:border-indigo-500 dark:focus:border-indigo-400 text-slate-800 dark:text-slate-100 outline-none transition-all"
                >
                  <option value="">None (Top-level role)</option>
                  {roles
                    ?.filter(
                      (role: RoleResponse) => !editExcludedIds.has(role.id),
                    )
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl w-full max-w-sm p-6 text-center text-slate-800 dark:text-slate-100">
            ...
            <div className="flex gap-3 w-full">
              <button
                ...
                className="flex-1 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 font-medium transition-colors"
              >
                Cancel
              </button>
              <button
