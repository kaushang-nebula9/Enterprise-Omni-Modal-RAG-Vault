...
export const OrganisationSettingsPage: React.FC = () => {
  ...
  const [tier, setTier] = useState<"fast" | "balanced" | "powerful">(
    "balanced",
  );
  ...
  const [selectedProvider, setSelectedProvider] =
    useState<ProviderRegistryEntry | null>(null);
  ...
  const [isDefaultModelDropdownOpen, setIsDefaultModelDropdownOpen] =
    useState(false);
  ...
  useEffect(() => {
    const handleClickOutsideDefaultModel = (e: MouseEvent) => {
      if (
        defaultModelDropdownRef.current &&
        !defaultModelDropdownRef.current.contains(e.target as Node)
      ) {
    ...
    return () =>
      document.removeEventListener("mousedown", handleClickOutsideDefaultModel);
  }, []);
  ...
  const updateModelMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: any }) =>
      adminService.updateModel(id, data),
    onSuccess: () => {
      refetchModels();
      setIsAddModalOpen(false);
      resetForm();
  ...
  const handleSubmitModel = (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      input_cost_per_million_tokens:
        inputCost === "" ? null : parseFloat(inputCost),
      output_cost_per_million_tokens:
        outputCost === "" ? null : parseFloat(outputCost),
  ...
  useEffect(() => {
    if (orgData) {
      ...
      setBudgetLimit(
        orgData.monthly_budget_limit != null
          ? orgData.monthly_budget_limit.toString()
          : "",
      );
  ...
  const [generalUpdateStatus, setGeneralUpdateStatus] = useState<{
    type: "success" | "error";
    msg: string;
  } | null>(null);
  const [budgetUpdateStatus, setBudgetUpdateStatus] = useState<{
    type: "success" | "error";
    msg: string;
  } | null>(null);
  ...
  return (
    <div className="flex flex-col gap-6 w-full h-full pb-12 text-slate-800 dark:text-slate-100">
      <div className="shrink-0">
        <h1 className="text-2xl font-semibold font-sora text-slate-800 dark:text-slate-100">
          Organisation Settings
        </h1>
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
          Manage your organisation's settings, including general information,
          monthly budget, and available LLM models for RAG chat generation.
        </p>
      </div>
      ...
      <div className="flex flex-col gap-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <section className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-sm flex flex-col justify-between">
            <div>
              <h2 className="text-lg font-semibold text-slate-800 dark:text-slate-100 mb-4 font-sora">
                General
              </h2>
              <form
              ...
              >
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-indigo-100 dark:focus:ring-indigo-950/50 focus:border-indigo-500 dark:focus:border-indigo-400 text-slate-800 dark:text-slate-100 outline-none transition-all"
                  />
                </div>
                <div className="space-y-1">
                  <label className="text-sm font-medium text-slate-700 dark:text-slate-300">
                ...
                >
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      setIsDefaultModelDropdownOpen(!isDefaultModelDropdownOpen)
                    }
                  ...
                  >
                    <span>
                      {defaultModelId
                        ? modelsData?.find((m) => m.id === defaultModelId)
                            ?.display_name || defaultModelId
                        : "System Default (Oldest Active Anthropic Model)"}
                    </span>
                    <ChevronDown
                      className={`w-4 h-4 text-slate-400 transition-transform duration-200 ${isDefaultModelDropdownOpen ? "rotate-180" : ""}`}
                    />
                  </button>
                ...
                <button
                  ...
                  className="mt-2 w-fit px-6 bg-indigo-700 dark:bg-indigo-500 text-white rounded-lg py-2.5 font-medium hover:bg-indigo-600 dark:hover:bg-indigo-400 transition-colors disabled:opacity-50"
                >
              ...
              >
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    step="0.01"
                    min="0"
                    placeholder="e.g. 100.00 (Leave blank for no limit)"
                    value={budgetLimit}
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                Manage the available LLMs for RAG chat generation and system
                settings.
              </p>
            </div>
            <button
            ...
            <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-800 text-left border-collapse">
              <thead className="bg-slate-50 dark:bg-slate-800/40">
                <tr>
                  ...
                  <th className="px-4 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase text-right">
                    Actions
                  </th>
                </tr>
              </thead>
                      ...
                      <td className="px-4 py-3.5">
                        <span className="text-xs text-slate-650 dark:text-slate-400 font-medium">
                          {model.input_cost_per_million_tokens != null
                            ? `$${Number(model.input_cost_per_million_tokens).toFixed(2)}`
                            : "N/A"}{" "}
                          /{" "}
                          {model.output_cost_per_million_tokens != null
                            ? `$${Number(model.output_cost_per_million_tokens).toFixed(2)}`
                            : "N/A"}
                        </span>
                      </td>
                      <td className="px-4 py-3.5">
                      ...
                      <td className="px-4 py-3.5 text-right space-x-2">
                        <button
                          onClick={() => {
                            ...
                            const foundProv = providers.find(
                              (p) => p.provider_id === model.provider_id,
                            );
                            ...
                            setInputCost(
                              model.input_cost_per_million_tokens != null
                                ? model.input_cost_per_million_tokens.toString()
                                : "",
                            );
                            setOutputCost(
                              model.output_cost_per_million_tokens != null
                                ? model.output_cost_per_million_tokens.toString()
                                : "",
                            );
        ...
        <section className="border border-red-200 dark:border-red-950/60 rounded-xl p-6 bg-red-50 dark:bg-red-950/10 mb-10">
          <h2 className="text-lg font-semibold text-red-600 dark:text-red-400 mb-2 font-sora">
            Danger Zone
          </h2>
          <div className="flex flex-col md:flex-row gap-6 md:items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-600 dark:text-slate-400 max-w-lg mt-1">
                This will permanently delete your organisation, all members,
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl w-full max-w-md p-6 text-center text-slate-800 dark:text-slate-100">
            <h3 className="text-xl font-semibold mb-2 text-red-600 dark:text-red-400 font-sora">
              Are you absolutely sure?
            </h3>
            <p className="text-slate-600 dark:text-slate-400 mb-6 text-sm">
              This action cannot be undone. Type your organisation name{" "}
              <span className="font-semibold text-slate-800 dark:text-slate-200">
                ({orgName || "the current name"})
              </span>{" "}
              to confirm deletion.
            </p>
            <input
            ...
            <div className="flex gap-3 w-full">
              <button
                ...
                className="flex-1 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-300 hover:bg-slate-55 dark:hover:bg-slate-800 font-medium transition-colors"
              >
                Cancel
              </button>
              <button
                disabled={
                  deleteConfirmName !== orgName || deleteMutation.isPending
                }
            ...
            <form onSubmit={handleSubmitModel} className="space-y-4">
              ...
              <div className="space-y-1 relative" ref={providerDropdownRef}>
                ...
                >
                  ...
                  <ChevronDown
                    className={`w-4 h-4 text-slate-400 transition-transform duration-200 ${isProviderDropdownOpen ? "rotate-180" : ""}`}
                  />
                </button>
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  Friendly name shown across the app. Defaults to model name if
                  left blank.
                </p>
              </div>
              ...
              <div className="space-y-1 relative" ref={tierDropdownRef}>
                ...
                >
                  ...
                  <ChevronDown
                    className={`w-4 h-4 text-slate-400 transition-transform duration-200 ${isTierDropdownOpen ? "rotate-180" : ""}`}
                  />
                </button>
                    ...
                    {[
                      {
                        value: "fast",
                        label: "Fast",
                        desc: "Lightweight and cheap. Best for simple lookups and short questions.",
                      },
                      {
                        value: "balanced",
                        label: "Balanced",
                        desc: "General purpose. Handles summaries, comparisons, and multi-part questions.",
                      },
                      {
                        value: "powerful",
                        label: "Powerful",
                        desc: "Complex reasoning, long context, and multi-condition analysis.",
                      },
                    ].map((t) => (
                      <button
                        key={t.value}
                      ...
                      >
                        ...
                        <div className="text-xs text-slate-500 dark:text-slate-400 font-normal mt-0.5 leading-normal">
                          {t.desc}
                        </div>
                      </button>
                    ))}
