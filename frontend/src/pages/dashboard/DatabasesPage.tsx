...
export const DatabasesPage: React.FC = () => {
  ...
  const [isEngineFilterDropdownOpen, setIsEngineFilterDropdownOpen] =
    useState(false);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (
        engineFilterDropdownRef.current &&
        !engineFilterDropdownRef.current.contains(event.target as Node)
      ) {
  ...
  const grantAccessMutation = useMutation({
    mutationFn: ({ id, payload }: { id: string; payload: any }) =>
      databaseService.grantAccess(id, payload),
    onSuccess: () => {
      refetchAccess();
      setSelectedRoleIds([]);
      setSelectedDeptIds([]);
  ...
  const revokeAccessMutation = useMutation({
    mutationFn: ({ id, policyId }: { id: string; policyId: string }) =>
      databaseService.revokeAccess(id, policyId),
    onSuccess: () => {
      refetchAccess();
    },
  });
    ...
    try {
      ...
      if (connId && !password) {
        setTestResult({
          success: false,
          message:
            "Please enter the database password to run a connection test.",
        });
        setIsTesting(false);
        return;
      }
  ...
  return (
    <div className="space-y-6 text-slate-800 dark:text-slate-100 animate-in fade-in duration-300">
      ...
      <div className="flex justify-between items-center shrink-0">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
            Connect and manage external PostgreSQL and MySQL databases.
            Authorized employees can query them via chat.
          </p>
        </div>
      ...
      <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/50 rounded-2xl p-4 flex gap-3 text-sm text-amber-800 dark:text-amber-300">
        ...
        <div>
          <span className="font-semibold">Security Suggestion:</span> We highly
          recommend providing a <strong>read-only</strong> database user for
          connections. This adds an additional layer of security on your
          database host in addition to our application-level safety query
          bounds.
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center justify-end gap-3 w-full lg:w-auto shrink-0">
          ...
          <div
            ...
            ref={engineFilterDropdownRef}
          >
            <button
              onClick={() =>
                setIsEngineFilterDropdownOpen(!isEngineFilterDropdownOpen)
              }
              type="button"
            ...
            >
              ...
              <ChevronDown
                className={`w-4 h-4 text-slate-400 transition-transform duration-200 ${isEngineFilterDropdownOpen ? "rotate-180" : ""}`}
              />
            </button>
      ...
      {isDbsLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2].map((i) => (
            <div
              key={i}
              className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm animate-pulse h-48"
            />
          ))}
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-12 text-center shadow-sm">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1 max-w-sm mx-auto">
            Get started by adding your first external PostgreSQL or MySQL
            database connection.
          </p>
          <button
            onClick={handleOpenCreate}
              ...
              <div>
                ...
                <div className="flex justify-between items-start mb-4">
                  <div className="flex flex-row gap-3">
                    ...
                    >
                      <span
                        className={`w-1.5 h-1.5 rounded-full ${dbConn.status === "active" ? "bg-emerald-500" : "bg-rose-500"}`}
                      />
                  ...
                  <button
                    ...
                    disabled={
                      ...
                      refreshSchemaMutation.variables === dbConn.id
                    }
                  ...
                  >
                    <RefreshCw
                      className={`w-3.5 h-3.5 ${refreshSchemaMutation.isPending && refreshSchemaMutation.variables === dbConn.id ? "animate-spin" : ""}`}
                    />
                    Refresh
                  </button>
                </div>
                ...
                <div className="space-y-2 text-sm text-slate-500 dark:text-slate-400 border-t border-slate-100 dark:border-slate-800/50 pt-2 mb-4">
                  ...
                  <div className="flex justify-between">
                    <span>Tables:</span>
                    <span className="font-medium text-slate-800 dark:text-slate-200">
                      {dbConn.table_count ?? 0}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span>Last Sync:</span>
                    <span className="font-medium text-slate-800 dark:text-slate-200">
                      {formatDate(dbConn.last_synced_at)}
                    </span>
                  </div>
                  {dbConn.last_error && (
                    <div
                      className="text-xs text-rose-500 dark:text-rose-400 border-t border-rose-100 dark:border-rose-950/20 pt-2 mt-2 break-all line-clamp-2"
                      title={dbConn.last_error}
                    >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={() => {
                      if (
                        confirm(
                          `Are you sure you want to delete database "${dbConn.name}"?`,
                        )
                      ) {
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="pt-2 border-t border-slate-100 dark:border-slate-800 space-y-3">
                ...
                {testResult && (
                  <div
                    className={`p-3.5 rounded-xl border flex gap-2.5 text-sm ${
                      testResult.success
            ...
            <div className="flex justify-between items-center px-6 py-4 border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 rounded-t-2xl z-10">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  Define who can query this database and tables.
                </p>
              </div>
              <button
                  ...
                  <div>
                    ...
                    <div ref={roleDropdownRef} className="relative">
                      ...
                      >
                        ...
                        <ChevronDown
                          className={`w-4 h-4 text-slate-400 dark:text-slate-500 transition-transform duration-200 ${isRoleDropdownOpen ? "rotate-180" : ""}`}
                        />
                      </button>
                  ...
                  <div>
                    ...
                    <div ref={deptDropdownRef} className="relative">
                      ...
                      >
                        ...
                        <ChevronDown
                          className={`w-4 h-4 text-slate-400 dark:text-slate-500 transition-transform duration-200 ${isDeptDropdownOpen ? "rotate-180" : ""}`}
                        />
                      </button>
                  ...
                  <div>
                    ...
                    <div ref={tableDropdownRef} className="relative">
                      ...
                      >
                        ...
                        <ChevronDown
                          className={`w-4 h-4 text-slate-400 dark:text-slate-500 transition-transform duration-200 ${isTableDropdownOpen ? "rotate-180" : ""}`}
                        />
                      </button>
                  ...
                  <div>
                    ...
                    <div ref={colDropdownRef} className="relative">
                      ...
                      >
                        ...
                        <ChevronDown
                          className={`w-4 h-4 text-slate-400 dark:text-slate-500 transition-transform duration-200 ${isColDropdownOpen ? "rotate-180" : ""}`}
                        />
                      </button>
                        ...
                        <div className="absolute right-0 left-0 mt-1.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl z-20 max-h-60 overflow-y-auto p-2 space-y-1">
                          <div className="flex items-center justify-between px-2 py-1 border-b border-slate-100 dark:border-slate-800 pb-1.5 mb-1.5 text-xs text-indigo-650 dark:text-indigo-400 font-semibold">
                            <button
                              ...
                              onClick={() =>
                                setSelectedColumns(
                                  availableCols.map((c: any) => c.fullName),
                                )
                              }
                              className="hover:underline"
                            >
                              ...
                              return Object.keys(groups).map((tableName) => (
                                <div key={tableName} className="space-y-1">
                                  ...
                                  {groups[tableName].map((c: any) => {
                                    const isChecked = selectedColumns.includes(
                                      c.fullName,
                                    );
                                    return (
                                      <label
                                      ...
                                      >
                                        ...
                                        />
                                        <span
                                          className="text-slate-700 dark:text-slate-300 truncate font-mono text-xs"
                                          title={c.fullName}
                                        >
                                          {c.column}
                                        </span>
                  ...
                  <div className="border border-slate-150 dark:border-slate-800 rounded-xl overflow-hidden text-sm">
                    ...
                    <div className="divide-y divide-slate-150 dark:divide-slate-800">
                      {accessPolicies.map((policy) => (
                        <div
                          key={policy.id}
                          className="grid grid-cols-12 gap-2 px-4 py-3 items-center hover:bg-slate-50/50 dark:hover:bg-slate-900/10"
                        >
                          ...
                          <div className="col-span-3 text-slate-500 dark:text-slate-400 flex flex-col">
                            ...
                            {policy.granted_via === "inherited" && (
                              <span className="text-[10px] text-indigo-500">
                                via: {policy.inherited_from_role_name}
                              </span>
                            )}
                            {policy.granted_via === "department" && (
                              <span className="text-[10px] text-emerald-500">
                                dept: {policy.granted_via_department_name}
                              </span>
                            )}
                          </div>
