...
export const ReportsPage: React.FC = () => {
  ...
  const [statusFilter, setStatusFilter] = useState<
    "all" | "generating" | "complete" | "failed"
  >("all");
  const [typeFilter, setTypeFilter] = useState<
    "all" | "Documents" | "Database" | "Mixed"
  >("all");
  ...
  // Deletion state
  const [reportToDelete, setReportToDelete] = useState<ReportStatus | null>(
    null,
  );
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setReports((prev) =>
        prev.filter((r) => r.report_id !== reportToDelete.report_id),
      );
  ...
  const filteredReports = useMemo(() => {
    return reports.filter((r) => {
      ...
      if (searchQuery.trim()) {
        ...
        const matchesSession = (r.session_title || "")
          .toLowerCase()
          .includes(query);
      ...
      <div className="flex justify-between items-center shrink-0">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            View, download, and manage your custom structured documents
            generated from chat conversations.
          </p>
        </div>
      </div>
        ...
        <div className="bg-indigo-50 dark:bg-indigo-950/40 border border-indigo-100 dark:border-indigo-900/60 rounded-xl p-3.5 flex items-center justify-between shadow-sm shrink-0">
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm font-medium text-indigo-950 dark:text-indigo-200">
              Only showing reports generated from:{" "}
              <span className="font-bold underline">
                {preSelectedSessionTitle}
              </span>
            </p>
          </div>
          <button
          ...
          >
            ...
            {isStatusDropdownOpen && (
              <div className="absolute right-0 mt-1.5 w-44 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl z-30 overflow-hidden py-1">
                {(["all", "complete", "generating", "failed"] as const).map(
                  (statusVal) => (
                    <button
                      key={statusVal}
                      onClick={() => {
          ...
          >
            ...
            {isTypeDropdownOpen && (
              <div className="absolute right-0 mt-1.5 w-48 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl z-30 overflow-hidden py-1">
                {(["all", "Documents", "Database", "Mixed"] as const).map(
                  (typeVal) => (
                    <button
                      key={typeVal}
                      onClick={() => {
      ...
      <div className="flex-1 min-h-0 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden flex flex-col">
        {isLoading ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
            ...
            <h3 className="text-lg font-semibold text-slate-950 dark:text-slate-200 mt-3">
              An error occurred
            </h3>
            ...
            {reports.length === 0 ? (
              <>
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-2 leading-relaxed">
                  Reports are high-quality PDF summaries generated from your
                  database connections and documents by the Report Generation
                  Agent.
                </p>
                <p className="text-xs text-slate-400 dark:text-slate-500 mt-1 leading-normal">
                  To create a report, click the "Create Report" button in any
                  chat session.
                </p>
                <button
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-2">
                No reports match your current search queries or filters. Try
                adjusting them.
              </p>
            )}
          </div>
                    ...
                    <React.Fragment key={report.report_id}>
                        ...
                        >
                          <div className="flex items-center justify-end gap-2.5">
                            ...
                            {report.status === "complete" && (
                              <button
                                onClick={() =>
                                  handleDownload(report.report_id, report.title)
                                }
                      ...
                      {isExpanded && (
                        <tr>
                          <td
                            colSpan={6}
                            className="bg-slate-50/50 dark:bg-slate-950/30 p-6 border-b border-slate-200 dark:border-slate-800"
                          >
                                    ...
                                    {report.steps.map((step, idx) => {
                                      ...
                                      return (
                                        ...
                                        >
                                          {/* Connector Icon */}
                                          <div
                                            className={`absolute -left-[30px] top-0.5 w-5 h-5 rounded-full border bg-white dark:bg-slate-900 flex items-center justify-center shadow-sm transition-colors ${
                                              isStepFailed
      ...
      {reportToDelete && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-[100] flex items-center justify-center">
          <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 max-w-sm w-full mx-4 shadow-2xl text-slate-800 dark:text-slate-100 border border-slate-200 dark:border-slate-800">
            <h2 className="text-lg font-bold text-slate-900 dark:text-slate-100 font-sora">
              Delete Report
            </h2>
            <p className="text-slate-500 dark:text-slate-400 text-sm mt-2 leading-relaxed">
              Are you sure you want to delete "{reportToDelete.title}"? This
              will permanently remove the report from your list and delete its
              storage file.
            </p>
