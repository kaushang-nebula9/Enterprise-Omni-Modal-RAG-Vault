  ...
  const onSubmit = async (data: AcceptInviteFormValues) => {
    ...
    try {
      ...
      if (err.response && err.response.data && err.response.data.detail) {
        ...
        setApiError(
          "Failed to activate your account. The invite link may have expired or is invalid.",
        );
      }
    } finally {
      setIsLoading(false);
    ...
    return (
      <div className="w-full max-w-md animate-fade-in text-center">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mb-8">
          Your account has been set up successfully. You can now log in to the
          system.
        </p>
        <Link
          to="/login"
    ...
    return (
      <div className="w-full max-w-md animate-fade-in">
        <div className="bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-900/60 rounded-lg p-6 flex flex-col items-center text-center">
          ...
          <h3 className="font-sora text-xl font-bold text-red-800 dark:text-red-300 mb-2">
            Invalid Invite Link
          </h3>
          <p className="text-sm text-red-700 dark:text-red-400/90 mb-6">
            Invalid or missing invite link. Please contact your administrator to
            receive a valid invitation email.
          </p>
          <Link
            to="/login"
    ...
    <div className="w-full max-w-md animate-fade-in">
      <div className="mb-8">
        ...
        <p className="mt-2 text-sm text-slate-500 dark:text-slate-400 font-sans">
          You have been invited to join your organisation. Set a password to
